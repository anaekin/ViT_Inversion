...
def get_patch_prior_loss(inputs, patch_size=16):
    ...
    for k in range(1, h_patches):
        ...
        # upper_patch = inputs[:, k * patch_size, :, :]
        # lower_patch = inputs[:, (k - 1) * patch_size, :, :]
...
class ViTDeepInversionClass(object):
    ...
    def get_images(self, targets=None):
        ...
        warmup_1 = int(0.3 * self.n_iterations)  # Only CCE loss
        warmup_2 = int(0.6 * self.n_iterations)  # Add image prior loss
        ...
        for lr_it, lower_res in enumerate([1]):
            ...
            for iteration_loc in range(self.n_iterations):
                ...
                with torch.autocast(device_type="cuda", dtype=data_type):
                    ...
                    # Progressive Scaling for Losses
                    if iteration_loc <= int(self.n_iterations / 2):
                        main_loss_scheduler = 1.0
                        image_prior_scheduler = 0.0
                        r_feature_scheduler = 0.0
                    else:
                        main_loss_scheduler = 0.5
                        image_prior_scheduler = 1.0
                        r_feature_scheduler = 1.0
                    ...
                    # Image Prior Loss (from GradViT paper)
                    ...
                    # Extra Prior Losses (TV_L1, TV_L2, L2)
                    ...
                    # Final Loss Function
                    loss = (
                        (self.main_loss_scale * main_loss_scheduler * loss_criterion)
                        + (self.r_feature_scale * r_feature_scheduler * loss_r_feature)
                        + (
                            self.image_prior_scale
                            * image_prior_scheduler
                            * loss_image_prior
                        )
                        + loss_aux
                    )
